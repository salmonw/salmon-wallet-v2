...
const {
  createMany: createNetworkAccounts,
} = require('./network-account-factory');
...
const create = async ({
  ...
  const enabledNetworks = networks.filter(
    network => switches[network.id]?.enable,
  );

  await Promise.all(
    enabledNetworks.map(async network => {
      const indexes = pathIndexes[network.id] || [0];
      networksAccounts[network.id] = await createNetworkAccounts({
        network,
        mnemonic,
        indexes,
      });
