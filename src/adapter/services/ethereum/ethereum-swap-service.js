...
const calculateFee = trade => {
  ...
  if (uniswapVersion === UniswapVersion.v3) {
    return {
      percent:
        liquidityProviderFeePercentsV3.reduce(
          (value, fee) => value + fee - value * fee,
          0,
        ) * 100,
    };
  }
...
const quote = async (connection, inToken, outToken, amount, slippage) => {
  ...
  const wEthAddress = removeEthFromContractAddress(
    ETH.info(chainId).contractAddress,
  );

  if (inToken === ETH_ADDRESS && outToken === wEthAddress) {
    const tokensFactory = new TokensFactoryPublic({
      ethereumProvider: connection.provider,
    });
...
const execute = async (connection, quote) => {
  ...
  if (trade.approvalTransaction) {
    const approved = await connection.sendTransaction(
