...
const buildERC20Transfer = async (
  connection,
  network,
  address,
  amount,
  ...
  const { decimals } =
    (await getTokenByAddress(network, address))?.[0] || opts || {};
...
const estimateGas = async (connection, network, to, token, amount, opts) => {
  ...
  if (opts.contract) {
    ...
    if (standard === 'ERC1155') {
      const { contract, from, data } = await buildERC1155Transfer(
        connection,
        opts,
      );
  ...
  const { contract, value } = await buildERC20Transfer(
    connection,
    network,
    token,
    amount,
...
const createTransaction = async (
  ...
  if (opts.contract) {
    ...
    if (standard === 'ERC1155') {
      ...
      const { hash } = await contract.safeTransferFrom(
        from,
        to,
        token,
        amount,
