...
const transferStorageDeposit = async (
  connection,
  contractName,
  receiverId,
  amount,
...
const quote = async (connection, network, tokenInId, tokenOutId, amount) => {
  ...
  if (tokenIn.id === nearTokenId && tokenOut.id === nearTokenId) {
    return {
      routeNames: [],
      routeSymbols: [
        mapSymbol(tokenInId, tokenIn),
        mapSymbol(tokenOutId, tokenOut),
      ],
  ...
  const pool = pools
    ...
    .reduce((prev, current) =>
      prev.amounts[1] > current.amounts[1] ? prev : current,
    );

  const amountOut = await connection.viewFunction(
...
const swap = async (connection, network, accountId, quote) => {
  ...
  const storageBalance = await getStorageBalance(
    connection,
    tokenOut.id,
    accountId,
  );
  ...
  if (storageBalance?.total === undefined) {
    try {
      ...
      if (
        e.message ===
        'attached deposit is less than the mimimum storage balance'
      ) {
        await transferStorageDeposit(
          connection,
          tokenOut.id,
